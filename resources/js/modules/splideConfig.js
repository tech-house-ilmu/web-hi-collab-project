export function initSplides() {
    ...
    const merchSplideEl = document.querySelector("#merchSplide");
    if (merchSplideEl) {
        const merchSplide = new Splide(merchSplideEl, {
            type: "loop",
            perPage: 1,
            perMove: 1,
            gap: "0px",
        ...
        merchSplide.on("mounted", () => {
            const pagination = merchSplideEl.querySelector(
                ".splide__pagination"
            );
            if (pagination) {
                pagination.classList.add(
                    ...
                    "justify-center",
                    "gap-2"
                ); // tailwind margin-top 1rem
            }
        });

        if (!merchSplide.state.is(Splide.STATES.MOUNTED)) {
            merchSplide.mount();
        }
    }
